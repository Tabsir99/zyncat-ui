...
| 3   | angle E, the registry blurb's "first +N"                                                            | `text={... selectedOptions[0].label}` and `count={selectedOptions.length - 1}` in `MultiSelect.tsx:130-132`; `{count > 0 && <span className="zc-select__count">+{count}</span>}` in `core/trigger.tsx:76`; `placeholder = 'Select options'`                                                                                                   |
