  ...
  return (
    <>
      ...
      <Demo label="error · warning · success">
        ...
        <TextField id="warn" label="Display name" warning="This is already taken." value="acme" readOnly style={{ width: W }} />
  ...
  return (
    <>
      <Demo label="unit · bounds · helper">
        <NumberField id="seats" label="Seats" unit="users" min={1} max={50} value={seats} onChange={setSeats} helper="1–50 seats." style={{ width: W }} />
      </Demo>
      <Demo label="error · sizes">
        <NumberField id="pp" label="Items per page" min={0} max={10} value={perPage} onChange={setPerPage} error={perPage > 8 ? 'Keep it under 8.' : undefined} style={{ width: W }} />
  ...
  return (
    <>
      <Demo label="label · description">
        <div className="stack">
          <Checkbox label="Email me about product updates" checked={updates} onChange={(e) => setUpdates(e.target.checked)} />
          <Checkbox size="sm" label="Enable two-factor auth" description="Adds a code prompt at sign-in." checked={twofa} onChange={(e) => setTwofa(e.target.checked)} />
        </div>
      </Demo>
      <Demo label="indeterminate · invalid · disabled">
        <div className="stack">
          <Checkbox label="Select all" indeterminate onChange={() => {}} />
          <Checkbox label="I agree to the terms" invalid={!agree} required checked={agree} onChange={(e) => setAgree(e.target.checked)} />
    ...
    <>
      <Demo label="label · description">
        <div className="stack">
          ...
          <Toggle size="sm" label="Public profile" description="Anyone with the link can view." checked={pub} onChange={(e) => setPub(e.target.checked)} />
        </div>
      </Demo>
  ...
  return (
    <>
      <Demo label="rows · helper · disabled option">
        <RadioGroup name="role" value={role} onChange={setRole} label="Member role" helper="Controls what they can access." options={ROLES} style={{ width: W }} />
      </Demo>
      <Demo label="cards · horizontal · icons">
        <RadioGroup name="plan" variant="cards" orientation="horizontal" value={plan} onChange={setPlan} label="Plan" options={PLAN} />
      </Demo>
  ...
  return (
    <>
      ...
      <Demo label="searchable">
        <div style={{ width: W }}>
          <MultiSelect options={PEOPLE} value={reviewers} onChange={(v) => setReviewers(v)} searchable searchPlaceholder="Filter people" placeholder="Add reviewers" ariaLabel="Reviewers" />
        </div>
      </Demo>
