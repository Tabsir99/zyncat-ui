...
import { DOCS, GROUPS, type Doc } from './registry';
import { Breadcrumbs, CodeBlock, InstallationBox, ExampleCard } from './kit';
    ...
    <div className="doc-layout">
      <article className="page">
            ...
            {heroTab === 'preview' ? (
              <div className="hero-preview__canvas" key={heroKey}>
                <div className="hero-preview__inner">
                  <ClientOnly>{() => <HeroComponent />}</ClientOnly>
                </div>
              </div>
            ) : heroCode ? (
            ...
            <div className="examples-list">
              {examples.map((ex) => (
                <div key={ex.id} className="example-block" id={`example-${ex.id}`}>
                  ...
                  <ClientOnly>{() => <ExampleCard Component={ex.Component} code={ex.code} />}</ClientOnly>
                </div>
              ))}
            </div>
          </section>
        ...
        <nav className="page-pagination" aria-label="Component navigation">
          {prevDoc ? (
            <Link to={`/${prevDoc.slug}`} className="pagination-card pagination-card--prev">
          ...
          {nextDoc ? (
            <Link to={`/${nextDoc.slug}`} className="pagination-card pagination-card--next">
              <span className="pagination-card__sub">
