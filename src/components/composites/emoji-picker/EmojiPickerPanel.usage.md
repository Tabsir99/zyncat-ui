...
once before the panel first opens or it throws; onEmojiDataLoaded(cb) fires when it lands.
getEmojiUrl is REQUIRED. Two shapes. search renders the panel's own field, opened from trigger.
query drives the grid from an input YOU own - point popoverProps.anchor at the caret rect and
forward keys through the ref.
